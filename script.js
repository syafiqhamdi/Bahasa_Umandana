...
function normalize(str) {
  ...
  for (var i = 0; i < str.length; i++) {
    if (str[i] + str[i + 1] + str[i + 2] + str[i + 3] + str[i + 4] == 'aiden') {
      ...
      i += 4;
      continue;
    } else if (str[i] + str[i + 1] + str[i + 2] + str[i + 3] + str[i + 4] == 'epre') {
      newStr.push('e')
      i += 4;
      continue;
    } else if (str[i] + str[i + 1] + str[i + 2] + str[i + 3] + str[i + 4] == 'ipri') {
      newStr.push('i');
      i += 4;
      continue;
    } else if (str[i] + str[i + 1] + str[i + 2] + str[i + 3] + str[i + 4] == 'opro') {
      newStr.push('o');
      i += 4;
      continue;
    } else if (str[i] + str[i + 1] + str[i + 2] + str[i + 3] + str[i + 4] == 'upru') {
      newStr.push('u');
      i += 4;
      continue;
